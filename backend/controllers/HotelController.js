...
const amadeus = new Amadeus({
  ...
  const saveBooking = async (req, res) => {
    const { hotelid, hotelname, cityCode, singleNumber, doubleNumber, tripleNumber, checkIn, checkOut, pricing, status, paymentMethod } = req.body;
    ...
    try {
        ...
        if (paymentMethod === 'wallet') {
            ...
            const session = await stripe.checkout.sessions.create({
                ...
                mode: 'payment',
                success_url: `${process.env.FRONTEND_URL}/successHotel?session_id={CHECKOUT_SESSION_ID}&hotelid=${encodeURIComponent(hotelid)}&hotelname=${encodeURIComponent(hotelname)}&cityCode=${encodeURIComponent(cityCode)}&singleNumber=${singleNumber}&doubleNumber=${doubleNumber}&tripleNumber=${tripleNumber}&checkIn=${encodeURIComponent(checkIn)}&checkOut=${encodeURIComponent(checkOut)}&pricing=${pricing}&paymentMethod=${paymentMethod}`,
